    ...
    for (ui32 centroid_idx = 1; centroid_idx < centroid_count; ++centroid_idx) {
        ...
        for (ui32 member_idx = 0; member_idx < member_count; ++member_idx) {
            ...
            if (member_chosen_as_centroid[member_idx]) {
                cumulative_distance_2s[member_idx]
                    = cumulative_distance_2s[member_idx - 1];
                continue;
            }

            // Calculate distance to nearest chosen centroid.
            Precision minimum_distance_2_to_chosen_centroids
                = std::numeric_limits<Precision>::max();
            for (ui32 chosen_centroid_idx = 0; chosen_centroid_idx < centroid_idx;
                 ++chosen_centroid_idx)
            {
                Precision distance_2_to_chosen_centroid = member_distance_2(
                    members[member_idx], centroids[chosen_centroid_idx]
                );
...
cluster::impl::NearestCentroid<Precision> cluster::impl::nearest_centroid(
    ...
    Precision new_distance_2_to_current_cluster = member_distance_2(
        member, clusters[member_metadata.current_cluster.idx].centroid
    );
    if (new_distance_2_to_current_cluster < member_metadata.current_cluster.distance) {
        return NearestCentroid<Precision>{ member_metadata.current_cluster.idx,
                                           new_distance_2_to_current_cluster };
    }

    NearestCentroid<Precision> nearest_centroid
        = { member_metadata.current_cluster.idx, new_distance_2_to_current_cluster };

    for (ui32 cluster_idx = 0; cluster_idx < cluster_count; ++cluster_idx) {
        if (cluster_idx == member_metadata.current_cluster.idx) continue;

        Precision centroid_distance_2
            = member_distance_2(member, clusters[cluster_idx].centroid);
...
void cluster::k_means(
    ...
    impl::MemberClusterMetadata<Precision>* member_cluster_metadata
        = new impl::MemberClusterMetadata<Precision>[member_count];
    ...
    [[maybe_unused]] impl::NearestCentroidList* nearest_centroids_lists
        = new impl::NearestCentroidList[member_count];
    ...
    do {
        ...
        // For each member, determine which centroid it is nearest to and add its
        // position to a new centroid which will then take its position as the average
        // position of the associated members.
        //
             ...
             ++initial_cluster_idx)
        {
            ...
            for (ui32 in_cluster_member_idx = 0;
                 in_cluster_member_idx
                 < initial_clusters[initial_cluster_idx].member_count;
                 ++in_cluster_member_idx)
            {
                // Set metadata at start of k_means algorithm.
                if (iterations == 0) {
                    member_cluster_metadata[global_member_idx].initial_member_idx
                        = in_cluster_member_idx;
                    member_cluster_metadata[global_member_idx].initial_cluster_idx
                        = initial_cluster_idx;
                    member_cluster_metadata[global_member_idx].current_cluster.idx
                        = initial_cluster_idx;
                        ...
                        || options.no_approaching_centroid_optimisation)
                    {
                        ...
                        member_cluster_metadata[global_member_idx]
                            .current_cluster.distance
                            = member_distance_2(
                                cluster.members[in_cluster_member_idx], cluster.centroid
                            );
                    }
                }
                ...
                if (options.no_centroid_subset_optimisation || iterations == 0) {
                    ...
                    if (iterations == 1) {
                        impl::NearestCentroidAndList<Precision>
                            nearest_centroid_and_list
                ...
                // If this is the first member to join a cluster this round, then set
                ...
                if (!cluster_modified_in_iteration[initial_cluster_idx]) {
                    clusters[nearest_centroid.idx].centroid.x
                        = cluster.members[in_cluster_member_idx].x;
                    clusters[nearest_centroid.idx].centroid.y
                        = cluster.members[in_cluster_member_idx].y;
                    clusters[nearest_centroid.idx].centroid.z
                        = cluster.members[in_cluster_member_idx].z;
                ...
                // If the member has changed cluster membership, then update changes in
                // iteration and its current cluster index.
                if (member_cluster_metadata[global_member_idx].current_cluster.idx
                ...
                if (options.no_approaching_centroid_optimisation) {
                    member_cluster_metadata[global_member_idx].current_cluster.distance
                        = std::numeric_limits<Precision>::min();
                } else {
                    member_cluster_metadata[global_member_idx].current_cluster.distance
    ...
    //                        - sorted member buffers (on distance to centroid, e.g.);
    //                        or,
    //                        - on-demand sorting, taking ownership of previous member
    //                        buffers and extracting changed members and placing in
    //                        others.

    // Allocate necessary buffers for clusters.
    for (ui32 cluster_idx = 0; cluster_idx < cluster_count; ++cluster_idx) {
        clusters[cluster_idx].members
            = new Member<Precision>[clusters[cluster_idx].member_count];
    }
    ...
    for (ui32 member_idx = 0; member_idx < member_count; ++member_idx) {
        ui32 new_cluster_offset
            = member_cursors[member_cluster_metadata[member_idx].current_cluster_idx]++;

        const Cluster<Precision>& old_cluster
            = initial_clusters[member_cluster_metadata[member_idx].initial_cluster_idx];
        const Cluster<Precision>& new_cluster
            = clusters[member_cluster_metadata[member_idx].current_cluster_idx];

        new_cluster.members[new_cluster_offset]
            = old_cluster
                  .members[member_cluster_metadata[member_idx].initial_member_idx];
    }
