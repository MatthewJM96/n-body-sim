...
public:
     ...
     * \param box The box in which to place the distribution of particles.
     *
     * \return The number of particles actually placed into the buffer.
     */
    virtual ui32 generate(Particle<Precision>* particles, ui32 target_particle_count, Volume<Precision> box) = 0;
};
...
public:
     ...
     * \return The number of particles actually placed into the buffer.
     */
    virtual ui32 generate(Particle<Precision>* particles, ui32 target_particle_count, Volume<Precision> box) override {
        ui32 target_particle_count_on_axis = std::floor(std::cbrtf((float)target_particle_count));
        ...
        for (ui32 x = 0; x < target_particle_count_on_axis; ++x) {
            for (ui32 y = 0; y < target_particle_count_on_axis; ++y) {
                for (ui32 z = 0; z < target_particle_count_on_axis; ++z) {
                    ui32 buffer_idx = x * target_particle_count_on_axis * target_particle_count_on_axis + y * target_particle_count_on_axis + z;

                    if (buffer_idx + 1 >= target_particle_count) return target_particle_count;
                    ...
                    particles[buffer_idx] = Particle<Precision>{x_ratio * box.x, y_ratio * box.y, z_ratio * box.z};
                }
            }
        }
